...
export class RouterManagementService {
  ...
  /**
   * Toggles PPPoE user enabled state on the router and synchronizes their profile.
   * If direct RouterOS API credentials are configured, performs the operation directly with instant active session kick.
   * Otherwise, falls back to queueing the command for the 15-second agent sync script.
   */
  async togglePPPoEUserState(opts: {
    tenantId: string;
    routerId: string;
    username: string;
    enabled: boolean;
    ...
    const { tenantId, routerId, username, enabled, profileName, rateLimit } = opts;
    console.log(`[RouterManagement] Toggling PPPoE user ${username} state to enabled=${enabled} on router ${routerId}`);
    ...
    try {
      ...
      const routerHost = router?.ip_address || router?.public_ip;
      if (router && !router.is_disabled && routerHost && (router.api_password || router.api_username)) {
        console.log(`[RouterManagement] Direct API connection for toggling/syncing ${username} on ${routerHost}...`);
        const { MikrotikApiClient } = await import("@/lib/mikrotik-api-client.server");
        const client = new MikrotikApiClient({
          host: routerHost,
          port: router.api_port || 8728,
        ...
        if (profileName) {
          // If a profile name is provided, use the comprehensive sync method to update both profile and state
          await client.syncPPPoEUserProfileAndStatus(username, profileName, rateLimit, enabled);
        } else {
          // Otherwise simply toggle the enabled state
          await client.setPPPoEUserEnabled(username, enabled);
        }

        directApiSuccess = true;
        console.log(`[RouterManagement] Direct API successfully toggled/synced PPPoE user ${username} on ${router.name}`);
        await client.disconnect();
      }
    } catch (err: any) {
      directApiError = err?.message || String(err);
      console.warn(`[RouterManagement] Direct API toggle/sync failed: ${directApiError}. Enqueuing command.`);
    }
    ...
    return this.enqueueCommand({
      ...
      payload: {
        username,
        disabled: false, // Always keep secret enabled so they can connect and be redirected to wifibilling.site to make payments!
        profile: enabled ? (profileName || "default") : "expired-limited",
      },
  ...
  async setupHighCapacityPPPoEPool(tenantId: string, routerId: string) {
    ...
    return this.enqueueCommand({
      ...
      payload: {
        ...
        activeRange: "172.16.0.2-172.31.255.254",
        activeCapacity: 1048574,
        expiredPool: "expired_pppoe_pool",
        expiredRange: "10.250.0.2-10.250.255.254",
        expiredCapacity: 65534,
        localAddress: "10.0.0.1",
        natSubnet: "172.16.0.0/12",
        expiredNatSubnet: "10.250.0.0/16",
      },
      status: directApiSuccess ? "done" : "queued",
      directApiSuccess,
      directApiError,
