...
export const getGithubConfig = createServerFn({ method: "GET" })
  .middleware([requireSupabaseAuth])
  .handler(async ({ context }) => {
    await assertSuperAdmin(context.supabase, context.userId, context.claims);
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
    ...
    return {
      githubRepo: (value.github_repo as string) || "",
      githubBranch: (value.github_branch as string) || "main",
      githubTokenMasked: mask(value.github_token as string),
    };
  });
...
const githubConfigSchema = z.object({
  ...
  githubBranch: z.string().max(100).optional(),
});

export const saveGithubConfig = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((input: unknown) => githubConfigSchema.parse(input))
  .handler(async ({ data, context }) => {
    await assertSuperAdmin(context.supabase, context.userId, context.claims);
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
    const db = supabaseAdmin;
    ...
    const token = data.githubToken?.trim()
      ...
      : (existingValue.github_token as string) || "";

    const newValue = {
      github_token: token,
      github_repo: data.githubRepo?.trim() || "",
      github_branch: data.githubBranch?.trim() || "main",
    };

    const { error } = await db.from("app_configurations").upsert(
      {
        key: "github_push_config",
        value: newValue,
        description: "Configuration for self-pushing the codebase to GitHub",
...
const triggerPushSchema = z.object({
  forcePush: z.boolean(),
});

export const triggerGithubPush = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((input: unknown) => triggerPushSchema.parse(input))
  .handler(async ({ data, context }) => {
    await assertSuperAdmin(context.supabase, context.userId, context.claims);
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
    const db = supabaseAdmin;

    const { data: configRecord } = await db
      .from("app_configurations")
      .select("value")
      .eq("key", "github_push_config")
      .maybeSingle();
    ...
    const repo = config.github_repo as string;
    const branch = (config.github_branch as string) || "main";
    ...
    try {
      ...
      // 2. Setup user config
      try {
        execSync("git config user.name 'Platform Admin'", { cwd: rootDir });
        execSync("git config user.email 'admin@platform.com'", { cwd: rootDir });
      } catch (err) {
      ...
      if (status) {
        ...
        execSync('git commit -m "Self-push update from Platform Admin Dashboard"', {
          cwd: rootDir,
        });
      } else {
        ...
        const commitCount = execSync("git rev-list --all --count || echo 0", { cwd: rootDir })
          ...
          .trim();
        if (commitCount === "0") {
          logOutput += "[Git] Creating initial commit...\n";
          execSync('git commit -m "Initial commit from Platform Admin Dashboard"', {
            cwd: rootDir,
          });
        } else {
      ...
      const command = `git push ${forceFlag} "${authUrl}" HEAD:${branch}`;
      ...
      try {
        ...
        logOutput += stdout || "Pushed successfully.\n";
        return {
          ok: true,
          message: "Self-push completed successfully!",
          output: logOutput,
        };
      } catch (err: any) {
        ...
        throw new Error(`Git push failed: ${sanitizedMsg}`);
      }
    } catch (err: any) {
      throw new Error(err.message || "Failed to execute git push operations.");
    }
  });

export const getSubscriptionLogs = createServerFn({ method: "GET" })
  .middleware([requireSupabaseAuth])
  .handler(async ({ context }) => {
    await assertSuperAdmin(context.supabase, context.userId, context.claims);
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
    ...
    const { data: logs, error } = await db
      .from("audit_logs")
      .select("id, tenant_id, action, entity_id, metadata, created_at, tenants(name, slug)")
    ...
    return (logs ?? []).map((l: any) => ({
      ...
      tenantName: l.tenants?.name || "Unknown Tenant",
      tenantSlug: l.tenants?.slug || "",
      action: l.action,
      transactionId: l.entity_id,
      receipt: l.metadata?.receipt || null,
      days: l.metadata?.days || 30,
      activatedAt: l.metadata?.activated_at || l.created_at,
      nextEnd: l.metadata?.next_end || null,
