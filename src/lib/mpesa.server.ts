...
export async function resolveCallbackUrl(
  ...
  try {
    const { data } = await db
      ...
      .maybeSingle();
    storedBase =
      (data as { callback_base_url?: string | null } | null)?.callback_base_url?.trim() ?? "";
    if (storedBase && !storedBase.startsWith("http://") && !storedBase.startsWith("https://")) {
      storedBase = `https://${storedBase}`;
    }
  } catch {
    // Ignore DB error
  }
  ...
  try {
    ...
    if (req) {
      ...
      if (!requestCandidate && host && !host.includes("localhost") && !host.includes("127.0.0.1")) {
        requestCandidate = `${proto === "http" ? "https" : proto}://${host}`;
      }
    }
  } catch {
  ...
  if (requestOrigin) {
    try {
      ...
      if (u.hostname && u.hostname !== "localhost" && u.hostname !== "127.0.0.1") {
        explicitCandidate = `https://${u.host}`;
      }
    } catch {
      /* ignore */
    }
  ...
  let base =
    candidates
      ...
      .find((c) => isDarajaSafeBase(c)) ?? "";

  if (!base) {
    for (const raw of [requestCandidate, explicitCandidate, storedBase]) {
      if (!raw) continue;
      try {
        const u = new URL(raw);
  ...
  if (!base) {
    throw new Error(
      "M-Pesa CallBackURL missing. Please specify your public website URL in Super Admin -> Platform M-Pesa settings.",
    );
  }
  ...
  try {
    ...
    base = base.replace(/\/api\/public\/mpesa\/callback\/?$/i, "").replace(/\/+$/, "");
  }

  return `${base}/api/public/mpesa/callback`;
}
...
export async function stkPushQuery(
  ...
  try {
    ...
    if (!res.ok) {
      if (data.errorCode === "500.001.1001" || data.errorCode === "500.001.1002") {
        return { resultCode: "pending", resultDesc: "Transaction is being processed", raw: data };
      }
      return {
        resultCode: "pending",
        resultDesc: (data.errorMessage as string) || (data.ResultDesc as string) || "Query failed",
        raw: data,
      };
    }

    return {
      resultCode: String(data.ResultCode ?? "pending"),
