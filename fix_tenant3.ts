...
async function run() {
  ...
  await supabase.from('tenants').update({ subscription_end_at: '2026-10-05T11:30:55.787Z' }).eq('id', tenantId);
  
  // Keep only the first audit log (postgres trigger), delete the rest
  const { data: audits } = await supabase.from('audit_logs').select('id, metadata').eq('tenant_id', tenantId).eq('action', 'subscription.renewed').order('created_at', { ascending: true });
